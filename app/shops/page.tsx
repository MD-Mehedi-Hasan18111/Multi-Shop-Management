      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {shops.map((shop: any) => {
          ...
          const CardContentComponent = (
            <Card className={`group border-none shadow-xl transition-all duration-500 rounded-[2rem] overflow-hidden bg-white dark:bg-zinc-900 ring-1 ring-zinc-100 dark:ring-zinc-800 ${isBlocked ? 'opacity-75 cursor-not-allowed' : 'hover:shadow-2xl'}`}>
              <div className="aspect-[2/1] bg-gradient-to-br from-zinc-100 to-zinc-200 dark:from-zinc-800 dark:to-zinc-900 relative overflow-hidden">
                {shop.bannerImages?.[0] ? (
                  <img src={shop.bannerImages[0]} alt="" className={`w-full h-full object-cover ${isBlocked ? '' : 'group-hover:scale-110'} transition-transform duration-700`} />
                ) : (
                ...
                {isBlocked && (
                  <div className="absolute top-4 right-4 bg-red-600 text-white text-xs font-black uppercase tracking-wider px-3 py-1.5 rounded-full shadow-lg z-10">
                    Not Available
                  </div>
                )}
              ...
              <CardContent className="pt-12 pb-8 px-8 space-y-4">
                ...
                <div className="pt-4 border-t border-zinc-50 dark:border-zinc-800 flex items-center justify-between">
                  <span className={`text-xs font-bold uppercase tracking-widest ${isBlocked ? 'text-red-500' : 'text-zinc-400'}`}>
                    {isBlocked ? "Not Available" : "View Store"}
                  </span>
                </div>
              </CardContent>
            </Card>
