...
export function ProductCard({ product, variant = "grid" }: ProductCardProps) {
  ...
  const isShopBlocked = product.shop?.isBlocked === true;
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (product.stock === 0 || isShopBlocked) return;
    dispatch(addItem({
      id: product._id,
      name: product.name,
      price: product.price,
  ...
  if (variant === "list") {
    return (
      <Card className={cn("group relative overflow-hidden transition-all border-muted/60 flex flex-col sm:flex-row h-full", isShopBlocked ? "opacity-75" : "hover:shadow-lg")}>
        <Link 
          href={isShopBlocked ? "#" : `/product/${product.slug}`} 
          className={cn("relative w-full sm:w-64 h-64 sm:h-auto shrink-0 bg-muted overflow-hidden", isShopBlocked && "cursor-not-allowed")}
          onClick={(e) => isShopBlocked && e.preventDefault()}
        >
          <Image
            ...
            className={cn("object-cover transition-transform", !isShopBlocked && "group-hover:scale-105")}
          />
          {discount > 0 && !isShopBlocked && (
            <Badge className="absolute top-4 left-4 bg-destructive">{discount}% OFF</Badge>
          )}
          {isShopBlocked && (
            <Badge className="absolute top-4 left-4 bg-red-600 text-white font-bold">Shop Not Available</Badge>
          )}
        </Link>
        ...
        <div className="flex-1 flex flex-col p-6">
          <div className="flex-1">
              ...
              {product.shop && (
                isShopBlocked ? (
                  <div className="flex items-center gap-2 text-zinc-400">
                    {product.shop.logo && (
                      <div className="w-5 h-5 rounded-full overflow-hidden border border-zinc-100 opacity-50">
            ...
            <Link 
              href={isShopBlocked ? "#" : `/product/${product.slug}`}
              className={cn(isShopBlocked && "cursor-not-allowed")}
              onClick={(e) => isShopBlocked && e.preventDefault()}
            >
              <h3 className={cn("text-2xl font-black mb-2 transition-colors", !isShopBlocked && "group-hover:text-primary")}>
                {product.name}
              </h3>
            </Link>
            ...
            <div className="flex items-center gap-4 mb-6">
              <span className={cn("text-3xl font-black", isShopBlocked ? "text-zinc-400" : "text-blue-600")}>BDT {product.price.toFixed(2)}</span>
          ...
          <div className="flex items-center gap-3 mt-auto">
            <Button 
              className={cn("flex-1 h-12 rounded-2xl font-bold transition-all text-base", 
                isShopBlocked ? "bg-zinc-100 text-zinc-400 cursor-not-allowed border border-zinc-200" :
              ...
              disabled={product.stock === 0 || isInCart || isShopBlocked} 
              onClick={handleAddToCart}
            >
              {!isShopBlocked && <ShoppingCart className="mr-2 h-5 w-5" />}
              {isShopBlocked ? "Not Available" : product.stock === 0 ? "Out of Stock" : isInCart ? "Already in Cart" : "Add to Cart"}
            </Button>
  ...
  return (
    <Card className={cn("group relative overflow-hidden transition-all border-muted/60 h-full flex flex-col", isShopBlocked ? "opacity-75" : "hover:shadow-lg")}>
      <Link 
        href={isShopBlocked ? "#" : `/product/${product.slug}`}
        className={cn(isShopBlocked && "cursor-not-allowed")}
        onClick={(e) => isShopBlocked && e.preventDefault()}
      >
        <div className="relative aspect-square overflow-hidden bg-muted">
          ...
          {isShopBlocked ? (
            <Badge className="absolute top-2 left-2 bg-red-600 text-white font-bold">Shop Not Available</Badge>
          ) : product.stock === 0 ? (
            <Badge variant="secondary" className="absolute inset-0 m-auto h-fit w-fit px-4 py-2 text-lg z-10">
              Out of Stock
            </Badge>
          ) : null}
      ...
      <CardContent className="p-4 flex-1">
          ...
          {product.shop && (
            isShopBlocked ? (
              <div className="flex items-center gap-1 text-zinc-400">
                {product.shop.logo && (
                  <div className="w-4 h-4 rounded-full overflow-hidden border border-zinc-100 opacity-50">
                    <img src={product.shop.logo} alt="" className="w-full h-full object-cover" />
                  </div>
                )}
                <span className="text-[10px] font-bold line-through truncate max-w-[80px]">
                  {product.shop.shopName}
                </span>
              </div>
            ) : (
        ...
        <Link 
          ...
          onClick={(e) => isShopBlocked && e.preventDefault()}
        >
          <h3 className={cn("font-semibold line-clamp-1 transition-colors", !isShopBlocked && "group-hover:text-primary")}>
            {product.name}
          </h3>
        </Link>
        <div className="mt-2 flex items-center gap-2">
          <span className={cn("text-lg font-bold", isShopBlocked ? "text-zinc-400" : "")}>BDT {product.price.toFixed(2)}</span>
          {product.comparePrice ? (
      ...
      <CardFooter className="p-4 pt-0 mt-auto">
        <Button 
          ...
          onClick={handleAddToCart}
        >
          {!isShopBlocked && <ShoppingCart className="mr-2 h-4 w-4" />}
          {isShopBlocked ? "Not Available" : product.stock === 0 ? "Out of Stock" : isInCart ? "Already in Cart" : "Add to Cart"}
        </Button>
      </CardFooter>
    </Card>
